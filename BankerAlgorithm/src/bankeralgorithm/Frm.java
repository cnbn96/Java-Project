...
public class Frm extends javax.swing.JFrame {
    ...
    static boolean safety(int a[], int all[][], int need[][], int r1, int c1) {
     ...
     public boolean requestFunction(int a[], int need[][], int all[][], int req[], int pick1, int p1,int r1){
        int pick = pick1;
        int p = p1;
        int r = r1;
        int avail[] = new int[r];
        ...
        if(flag){
            ...
            if(flag){
                ...
                if(safety(a, all, need, p, r)){
                    ...
                    lblResult.setText("Chuỗi hệ thống không an toàn");
                }
            }else{
                System.out.println("So wait");
                lblResult.setText("Phải chờ vì tài nguyên không sẵn có");
            }
        }else{
            System.out.println("Lỗi need < request");
            lblResult.setText("Lỗi need < request");
        }
        return false;
    }
    ...
    private void btnCheckActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCheckActionPerformed
        ...
        if(safety(avail, all, nd, r, c)){
            String s = "Hệ thống ở thời điểm Ti là an toàn,chuỗi an toàn là:";
            for(int i = 0;i<r;i++){
                s += safe[i]+",";
            }
            lblResult.setText(s.substring(0, s.length()));
        }else{
            lblResult.setText("Hệ thống không an toàn");
        }
        
        if((JOptionPane.showConfirmDialog(null,"Bạn có muốn Request: "))==JOptionPane.OK_OPTION){
            int pick;
            do{
                pick = Integer.parseInt(JOptionPane.showInputDialog("Process mà bạn muốn Request: "));
            }while(pick >= c);
            int rq[] = new int[r];
            for(int i=0;i<c;i++){
            ...
            if(requestFunction(avail,nd,all,rq,pick,r,c)){
                ...
                if(safety(avail, all, nd, r, c)){
                    String x = "Hệ thống ở thời điểm Ti là an toàn,chuỗi an toàn là:";
                    for(int i=0;i<r;i++){
                        x+=safe[i]+",";
                    }
                    JOptionPane.showMessageDialog(null, x.substring(0, x.length()));
                }else{
                    JOptionPane.showMessageDialog(null,"Hệ thống không an toàn");
                }
            }
        }
    ...
    private void btnProcessActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnProcessActionPerformed
        this.Processes = Integer.parseInt(JOptionPane.showInputDialog("số Process: "));
    ...
    private void btnResourceActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnResourceActionPerformed
        this.Resource = Integer.parseInt(JOptionPane.showInputDialog("số Resource: "));
